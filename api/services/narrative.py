...
【撰寫規則與量表方向說明（極為重要，務必嚴格遵守）】
1. 只能引用上述 JSON 中出現的數字與園名，**嚴禁自行推算、外插或杜撰任何未提供的數值、園名、行政區或事件**。
2. 【核心量表尺度定義（切勿混淆方向）】：
   - 本系統所有指標為「營運異常風險分數（Risk Score）」，數值為該園在全市的風險百分位，範圍 0 至 100 分。
   - **數值越高代表「風險越高、異常程度越嚴重、違規可能性越高、越危險」**！
   - **數值越低代表「風險越低、營運狀況越常態、越安全」**！
   - 分數 ≥ 90 分為「高風險（不可容忍群）」，即模型的優先稽查名單（全市前 10%），需列為優先突擊查核對象；30～89 分為「中風險」；< 30 分為「低風險（可容忍安全群）」。
   - 分數是相對排序而非絕對違規機率，敘述時不得將分數說成「違規機率為 X%」。
   - **嚴禁將「高分」敘述為「表現良好」「辦學績優」或「值得表揚」**；若某行政區或某同儕群組平均分數最高，代表該區或該群組「整體風險偏高，最需要加強稽查輔導」。
3. 風險分數為模型推估之相對異常指標，**不得敘述為既定違法事實、不得指控任何機構違法**，請使用「風險偏高」「預測違規機率較高」「建議優先查核」等行政審計用語。
4. 語氣為公文書面體，繁體中文，不使用 Markdown 標題或條列符號。
5. 執行摘要 3 至 5 句，需涵蓋受評家數、整體風險水準、最需關注之群組或行政區。
6. 關鍵發現 3 條，每條 40 字以內，須各自對應一項具體統計數字。
7. 稽查建議 3 條，每條 50 字以內，需具體可執行（如查核順序、跨機關會辦、資料補正）。
8. 若統計中顯示輿情涵蓋率偏低，請在摘要或建議中提醒該項限制。
...
def _template_narrative(stats: Dict[str, Any]) -> Dict[str, Any]:
    ...
    summary_parts = [
        ...
        f"其中風險百分位達 90 以上（模型優先稽查名單）之高風險機構計 {high} 所，歷史裁罰紀錄合計 {penalties} 件。",
    ]
    if top_group:
    ...
    recommendations = [
        "風險百分位 90 以上機構應依風險判斷基準列為不可容忍風險，由管理階層督導研擬改善計畫並提供資源。",
